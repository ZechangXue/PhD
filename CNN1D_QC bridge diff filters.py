...
def plot_all_curves(histories, filters_list):
    # 绘制准确率和损失图表
    plot_metric(histories, filters_list, 'accuracy', 'Training and Validation Accuracy', 'accuracy_plot.jpg')
    plot_metric(histories, filters_list, 'val_accuracy', 'Validation Accuracy', 'validation_accuracy_plot.jpg')
    plot_metric(histories, filters_list, 'loss', 'Training Loss', 'train_loss_plot.jpg')
    plot_metric(histories, filters_list, 'val_loss', 'Validation Loss', 'validation_loss_plot.jpg')
...
def run_experiment(repeats=1):
    ...
    for filters in filters_list:
        ...
        for r in range(repeats):
            _, history = evaluate_model(trainX, trainy, testX, testy, 11, filters)  # kernel_size 固定为 11
            history_list.append(history.history)
        histories.append(history_list)

    # 绘制所有曲线
    plot_all_curves(histories, filters_list)
