...
def evaluate_model(trainX, trainy, testX, testy, kernel_size):
	...
	model = Sequential()
	model.add(Conv1D(filters=5, kernel_size=kernel_size, strides=1, padding='same', activation='relu',
					 input_shape=(n_timesteps, n_features)))
	model.add(Conv1D(filters=5, kernel_size=kernel_size, padding='same', activation='relu'))
	...
	# 用adam
	adam = tf.keras.optimizers.Adam(lr=0.0005, beta_1=0.9, beta_2=0.999, epsilon=1e-8, decay=0)
...
def plot_metric(histories, kernel_sizes, metric, title, file_name):
    plt.figure(figsize=(15, 7))
    for i, history_list in enumerate(histories):
        for history in history_list:
            plt.plot(history[metric], label=f'{metric.title()} (Kernel={kernel_sizes[i]})')
...
# def plot_all_curves(histories, kernel_sizes):
#     # 绘制准确率和损失图表
#     plot_metric(histories, kernel_sizes, 'accuracy', 'Training and Validation Accuracy', 'accuracy_plot.jpg')
#     plot_metric(histories, kernel_sizes, 'val_accuracy', 'Validation Accuracy', 'validation_accuracy_plot.jpg')
#     plot_metric(histories, kernel_sizes, 'loss', 'Training Loss', 'train_loss_plot.jpg')
#     plot_metric(histories, kernel_sizes, 'val_loss', 'Validation Loss', 'validation_loss_plot.jpg')

def plot_all_curves(histories, Batch_size_list):
	# 初始化一个空的字典，用于存储所有数据
	data = {'Epoch': range(1, len(histories[0][0]['accuracy']) + 1)}
	metrics = ['accuracy', 'val_accuracy', 'loss', 'val_loss']

	for i, Batch_size in enumerate(Batch_size_list):
		for metric in metrics:
			# 对每个批量大小和指标，计算所有重复实验的平均值
			avg_metric = np.mean([history[metric] for history in histories[i]], axis=0)

			# 将数据添加到字典中，为每个度量创建一个唯一的列名
			data[f'{metric}_Batch_{Batch_size}'] = avg_metric
	...
	for metric in metrics:
		...
		for Batch_size in Batch_size_list:
			plt.plot(epochs, df[f'{metric}_Batch_{Batch_size}'], label=f'Kernel size {Batch_size}')
	...
	# 保存到Excel
	df.to_excel("metrics_summary_by_batch_size.xlsx", index=False)
...
def run_experiment(repeats=3):
    ...
    for kernel_size in kernel_sizes:
        ...
        for r in range(repeats):
            _, history = evaluate_model(trainX, trainy, testX, testy, kernel_size)
            history_list.append(history.history)  # 存储 history.history 而非 history 对象
        histories.append(history_list)

    # 绘制所有曲线
    plot_all_curves(histories, kernel_sizes)
