...
def evaluate_model(trainX, trainy, testX, testy, kernel_size, filters, Batch_size, learn):
	...
	# 用adam
	adam = tf.keras.optimizers.Adam(lr=learn, beta_1=0.9, beta_2=0.999, epsilon=1e-8, decay=0)
...
# def plot_metric(histories, filters_list, metric, title, file_name):
#     plt.figure(figsize=(15, 7))
#     for i, history_list in enumerate(histories):
#         for history in history_list:
#             plt.plot(history[metric], label=f'{metric.title()} (Filters={filters_list[i]})')
...
def plot_metric(histories, learn_list, metric, title, file_name):
    plt.figure(figsize=(15, 7))
    for i, history_list in enumerate(histories):
        for history in history_list:
            plt.plot(history[metric], label=f'{metric.title()} (lr={learn_list[i]})')
    plt.title(title, fontsize=16)
    plt.xlabel('Epoch', fontsize=14)
    plt.ylabel(metric.title(), fontsize=14)
    plt.legend(fontsize=14)
    ...
    plt.savefig(file_name)
    plt.close()
# def plot_all_curves(histories, filters_list):
...
# 			plt.plot(avg_metric, label=f'{metric.title()} (Filters={filters_list[i]})')
#
...
# 		plt.close()
# def plot_all_curves(histories, learn_list):
# 	for metric in ['accuracy', 'val_accuracy', 'loss', 'val_loss']:
# 		plt.figure(figsize=(15, 7))
# 		for i, history_list in enumerate(histories):
# 			# Calculate average for each epoch across all repeats
# 			avg_metric = np.mean([h[metric] for h in history_list], axis=0)
# 			plt.plot(avg_metric, label=f'{metric.title()} (lr={learn_list[i]})')
#
# 		plt.title(f'Training and Validation {metric.title()}', fontsize=16)
# 		plt.xlabel('Epoch', fontsize=14)
# 		plt.ylabel(metric.title(), fontsize=14)
...
def plot_all_curves(histories, Batch_size_list):
	...
	for metric in metrics:
		...
		for Batch_size in Batch_size_list:
			plt.plot(epochs, df[f'{metric}_Batch_{Batch_size}'], label=f'Learning rate {Batch_size}')
...
def run_experiment(repeats=3):
    learn_list = [0.0002, 0.00035, 0.0005]
    best_accuracy = 0
    best_model = None
    all_histories = []
    ...
    for learn_rate in learn_list:
        ...
        for _ in range(repeats):
            model, history = evaluate_model(trainX, trainy, testX, testy, 11, 7, 128, learn_rate)
            histories.append(history.history)

            # 计算当前模型的平均验证准确率
            current_avg_accuracy = np.mean(history.history['val_accuracy'])

            # 如果找到了更好的模型，则更新最佳准确率和模型
            if current_avg_accuracy > best_accuracy:
                best_accuracy = current_avg_accuracy
                best_model = model
    ...
    # 绘制所有平均曲线
    plot_all_curves(all_histories, learn_list)
