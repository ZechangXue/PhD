...
def load_dataset_group(group, label_group, prefix=''):
	...
	filenames = filenames1 + filenames2 + filenames3 + filenames4 + filenames5 + filenames6 + filenames7
	print(filenames)
	# filenames.sort()
	# load input data
...
def evaluate_model(trainX, trainy, testX, testy):
	...
	model.add(Conv1D(filters=7, kernel_size=11, strides=1, padding='same', activation='relu', input_shape=(n_timesteps, n_features)))

	model.add(Conv1D(filters=7, kernel_size=11, padding='same', activation='relu'))
	...
	# 用adam
	adam = tf.keras.optimizers.Adam(lr=0.00018, beta_1=0.9, beta_2=0.999, epsilon=1e-8, decay=0)
...
def plot_accuracy_bar_from_confusion(conf_mat):
    ...
    accuracies = diagonal / total_per_class * 100

    # 交换state1和state4的数值
    accuracies[[0, 3]] = accuracies[[3, 0]]
    ...
    # 将准确率缩放到0到99之间
    normalized_accuracies = np.clip(accuracies / accuracies.max() * 99, 0, 99)
    colors = [color_palette[int(a)] for a in normalized_accuracies]  # 根据每个state的准确率分配颜色
    ...
    plt.tight_layout()
    plt.savefig('accuracy_per_state.jpg')
